<?php
session_start();
if (!isset($_SESSION['usuario_id']) || !isset($_GET['id_empleado']) || !isset($_GET['fecha'])) {
...
$res_tickets = $conexion->query("
    SELECT tipo, moneda, monto 
    FROM historico_tickets 
    WHERE id_empleado = $id_empleado AND DATE(fecha) = '$fecha'
");
...
<head>
    <meta charset="UTF-8">
    <title>Ticket Jornada</title>
    <style>
        body { font-family: monospace; font-size: 12px; padding: 5px; margin: 0; }
        .watermark { position: absolute; top: 50px; left: 0; width: 100%; text-align: center; z-index: 0; }
        .watermark img { width: 140px; opacity: 0.04; }
        .content { position: relative; z-index: 1; }
        .center { text-align: center; }
        .line { border-top: 1px dashed #000; margin: 4px 0; }
        .linea-doble { border-top: 3px double #000; margin: 6px 0; }
        .detalle { margin-left: 10px; }
        .item-line { margin: 0; padding: 0; white-space: pre; }
        .resaltado-descuento { color: red; font-weight: bold; font-size: 13px; }
        .total-final { font-weight: bold; font-size: 14px; text-decoration: underline; }
    </style>
</head>
...
<div class="content">
    ...
    <strong>Fecha de Jornada:</strong> <?= $fecha ?><br>
    <div class="line"></div>
        ...
        <?php if (!empty($detalle[$tipo])): ?>
            ...
            <?php foreach ($detalle[$tipo] as $moneda => $items): ?>
                ...
                <?php foreach ($items as $item): ?>
                    <div class="item-line">
                        <?= str_pad(number_format($item['monto'], 0, ',', '.'), 6, ' ', STR_PAD_LEFT) ?> 
                        ------> <?= number_format($item['valor_cop'], 0, ',', '.') ?> COP
                    </div>
                <?php endforeach; ?>
            <?php endforeach; ?>
        <?php endif; ?>
